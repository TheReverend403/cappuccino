...
class NickServ(object):
    ...
    @irc3.event(r':(?P<ns>\S+)!\S+@\S+ NOTICE .* :This nickname is registered.*')
    ...
    @irc3.event(r':(?P<mask>\S+!\S+@\S+) NOTICE .* :Password accepted.*')
    def login_succeeded(self, mask):
        self.bot.log.info('Authenticated with {0}'.format(mask))

    @irc3.event(r':(?P<mask>\S+!\S+@\S+) NOTICE .* :Password incorrect.*')
