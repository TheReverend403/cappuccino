...
class Plugin(object):
    ...
    def sed(self, mask, target, sed):
        if target in self.history_buffer:
            ...
            try:
                message = edit(message, sed)
            except EditorException as error:
                self.bot.privmsg(target, '{0}: {1}'.format(mask.nick, error.args[0].decode('UTF-8')))
            else:
